...
if 'score' not in st.session_state:
    ...
    st.session_state.quiz_active = False # NEW: Quiz is not active initially, intro needs to play first.
...
if not df.empty:
    # Always display the review button. This allows reviewing *previous* quiz's missed answers.
    if st.button("Review Missed Answers"):
        st.session_state.review_mode = True
        st.session_state.quiz_active = False # Deactivate main quiz UI for review
        st.rerun() # Rerun to enter review mode and show content

    # Logic for Review Mode. If review_mode is True, display missed questions and stop.
    if st.session_state.review_mode:
        ...
        if not st.session_state.missed:
            ...
            for i, missed_q in enumerate(st.session_state.missed):
                st.markdown(f"**Question {i+1}:** {missed_q['question']}")
                st.markdown(f"Your answer: ❌ {missed_q['your_answer']}")
                st.markdown(f"Correct answer: ✅ {missed_q['correct_answer']}")
                st.markdown("---") # Separator for clarity
        # Offer a button to exit review mode and potentially start a new quiz
        if st.button("Back to Quiz / Start New Quiz"):
            st.session_state.review_mode = False
            # Reset everything to start a new quiz from level 1
            st.session_state.current_level = 1
            st.session_state.q_number = 1
            st.session_state.score = 0
            st.session_state.missed = [] # Clear missed upon explicit restart
            st.session_state.used_players = set()
            st.session_state.current_q = None
            st.session_state.awaiting_input = True
            st.session_state.show_intro = True
            st.session_state.quiz_complete = False
            # st.session_state.quiz_active will be set to True by the show_intro block after rerun
            st.rerun()
        st.stop() # Halt execution after displaying review, waiting for user interaction
    ...
    if st.session_state.q_number > 10:
        st.session_state.quiz_active = False # Deactivate quiz display while handling end-of-round state
        if st.session_state.score == 10 and st.session_state.current_level < 5:
            ...
            st.session_state.q_number = 1
            st.session_state.score = 0
            st.session_state.missed = [] # Clear missed upon perfect level completion
            ...
            # Offer to play again after mastering all levels
            if st.button("Play Again (Level 1)"):
                st.session_state.current_level = 1
                st.session_state.q_number = 1
                st.session_state.score = 0
                st.session_state.missed = [] # Clear missed for new game
                st.session_state.used_players = set()
                st.session_state.current_q = None
                st.session_state.awaiting_input = True
                st.session_state.show_intro = True
                st.session_state.quiz_complete = False # Not complete anymore, restarting
                # st.session_state.quiz_active will be set to True by the show_intro block after rerun
                st.rerun()
            st.stop() # Stop here until "Play Again" is clicked

        else:
            # Quiz complete, but score not 10 (and not max level)
            st.subheader("🏁 Quiz Complete!")
            st.write(f"Your final score: {st.session_state.score} out of 10")
            # Missed questions list is *not* cleared here, so it can be reviewed by "Review Missed Answers" button
            if st.button("Start New Quiz (Level 1)"):
                st.session_state.current_level = 1
                st.session_state.q_number = 1
                st.session_state.score = 0
                st.session_state.missed = [] # Clear missed upon explicit new quiz start
                st.session_state.used_players = set()
                st.session_state.current_q = None
                st.session_state.awaiting_input = True
                st.session_state.show_intro = True
                ...
                # st.session_state.quiz_active will be set to True by the show_intro block after rerun
                st.rerun()
            st.stop() # Halt execution, waiting for user to click a button


    # --- Level Intro Display ---
    # Show an introduction screen for the current level ONLY if show_intro is True,
    # and we are NOT already in quiz_active state (meaning transitioning from quiz end or start)
    # and NOT in review mode.
    if st.session_state.show_intro and not st.session_state.quiz_active and not st.session_state.review_mode:
        level_names = ['The Rook', 'No Slump Sophomore', 'Cap Space Problem', 'No All Star Break for You!', 'Knoxville Forever...']
        level_intros = [
        ...
        # After showing intro, set show_intro to False and ACTIVATE THE QUIZ
        st.session_state.show_intro = False # Don't show intro again until next level
        st.session_state.quiz_active = True # IMPORTANT: Activate the main quiz display
        st.rerun() # Trigger a rerun to move past the intro phase and display questions

    # --- Main Quiz UI (Questions and Options) ---
    # Only display quiz elements if quiz is active and not in review mode
    if st.session_state.quiz_active and not st.session_state.review_mode:
        ...
        # Safety check: If for some reason current_q is None, force a reset
        if 'current_q' not in st.session_state or st.session_state.current_q is None:
            st.warning("Could not generate a question. Resetting quiz.")
            # Full reset to restart from Level 1, ensuring a fresh start
            st.session_state.current_level = 1
            st.session_state.q_number = 1
            st.session_state.score = 0
            ...
            st.session_state.show_intro = True
            st.session_state.quiz_complete = False
            st.session_state.quiz_active = False # Deactivate quiz before intro
            st.rerun() # Rerun to display intro or first question
