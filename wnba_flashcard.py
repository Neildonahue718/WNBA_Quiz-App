...
if not df.empty:
    # Button to review missed answers. This must appear early in the script flow.
    # It allows reviewing *previous* quiz's missed answers even if a new quiz is not perfect.
    ...
    if st.session_state.q_number > 10:
        if st.session_state.score == 10 and st.session_state.current_level < 5:
            ...
            st.session_state.q_number = 1
            st.session_state.score = 0
            st.session_state.missed = [] # Clear missed for new level
            ...
            # Offer to start a new quiz. Do NOT clear missed list here initially.
            if st.button("Start New Quiz (Level 1)"):
                ...
                st.session_state.quiz_complete = False # Not complete anymore, restarting
                st.rerun()
            # Stop here. This allows the "Review Missed Answers" button (which is before this block)
            # to be clicked and show the *current* missed answers before they are cleared.
            st.stop()
